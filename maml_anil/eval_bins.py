...
def calculate_roc(thresholds,
    ...
    if pca == 0:
        ...
        logging.info(f"Current bounds of distance (adjust threshold accordingly): {np.min(dist):.4f} - {np.max(dist):.4f}")
...
def evaluate(embeddings, actual_issame, nrof_folds=10, pca=0, threshold_start=0, threshold_end=4, threshold_step=0.01):
